...
public class AmministratoreInterfaceMateriali extends javax.swing.JFrame {
    private Amministratore a = new Amministratore("","","");
    Query q = new Query();
    JTextField nome = new JTextField();
    JTextField nuovoNome = new JTextField();
    
    public AmministratoreInterfaceMateriali() {
        initComponents();
    }

    /*
    Hidden feature: interface for story ID 12
    ...
    */
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Materiali", "Competenze", "Utenti", "Siti", "Procedure", "Tipi" }));
        ...
        jLabelTitleAdmin.setText("Gestione materiali");
    ...
    private void aggiornaTabella() throws SQLException {
        ...
        LinkedList<Materiale> materials = this.a.viewMaterials();
        for (Materiale u : materials) {
    ...
    private void jButtonAggiungiMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButtonAggiungiMouseClicked
        Icon i = new ImageIcon(getClass().getResource("/images/material.png"));
        int r = JOptionPane.showConfirmDialog(this, this.jOptionAggiungi(), "Aggiungi materiale", JOptionPane.PLAIN_MESSAGE, JOptionPane.YES_OPTION, i);
        if (r == JOptionPane.YES_OPTION) {
            ...
            try {
                this.a.insertMaterial(nome.getText());
            } catch (SQLException ex) {
                i = new ImageIcon(getClass().getResource("/images/warning.png"));
            ...
            query = "select nomemateriale from materiali where nomemateriale = '" + nome.getText() + "'";
            try {
                ...
                while (test) {
                    if (rst.getString("nomemateriale").compareTo(nome.getText()) == 0) {
                        nomeTrovato = true;
                    }
                    test = rst.next();
                }
    ...
    private void jButtonEliminaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButtonEliminaMouseClicked
        ...
        int r = JOptionPane.showConfirmDialog(this, "Sei sicuro/a di voler eliminare il materiale?", "Eliminazione materiale", JOptionPane.YES_OPTION, JOptionPane.QUESTION_MESSAGE, i);
        String query = "";
        ResultSet rst = null;
        Boolean test = false;
        if (r == JOptionPane.YES_OPTION) {
            try {
                this.a.deleteMaterial(s);
                query = "select nomemateriale from materiali where nomemateriale = '" + s + "'";
                rst = Query.genericQuery(query);
                test = rst.next();
                if (!test) {
                    i = new ImageIcon(getClass().getResource("/images/ok.png"));
                ...
                while (test) {
                    if (rst.getString("nomemateriale").compareTo(s) == 0) {
                        i = new ImageIcon(getClass().getResource("/images/warning.png"));
    ...
    private void jButtonModificaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButtonModificaMouseClicked
        String s = this.jListLista.getSelectedValue();
        Icon i = new ImageIcon(getClass().getResource("/images/material.png"));
        int r = JOptionPane.showConfirmDialog(this, this.jOptionUpdate(), "Modifica materiale", JOptionPane.PLAIN_MESSAGE, JOptionPane.YES_OPTION, i);
        if (r == JOptionPane.YES_OPTION) {
            ...
            try {
                this.a.updateMaterial(s, nuovoNome.getText());
            } catch (SQLException ex) {
                i = new ImageIcon(getClass().getResource("/images/warning.png"));
                JOptionPane.showMessageDialog(this, "Errore nel database!", "Errore", JOptionPane.ERROR_MESSAGE, i);
                nuovoNome.setText("");
            ...
            query = "select nomemateriale from materiali where nomemateriale = '" + nuovoNome.getText() + "'";
            try {
                rst = Query.genericQuery(query);
                boolean test = rst.next();
                if (!test) {
                ...
                while (test) {
                    if (rst.getString("nomemateriale").compareTo(nuovoNome.getText()) == 0) {
                        nomeTrovato = true;
                    }
                    test = rst.next();
                }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AmministratoreInterfaceMateriali.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AmministratoreInterfaceMateriali().setVisible(true);
            }
        });
    }
